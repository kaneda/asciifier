...
appAPI.contextMenu.add("key1", "ASCII It!", function (data) {
  ...
  // Instead of drawing the whole image let's cap it at 800 pixels wide
  new_width = img.width;
  ...
  appAPI.db.async.get('ascii_dict', function(value) {
    if(value === null || value === undefined) { asciiDict = {}; }
    else { asciiDict = value; }
    ...
    */
    ...
    appAPI.db.async.set('ascii_dict',asciiDict, appAPI.time.yearsFromNow(10), function() {
        alert("Your art is ready; go to www.asciifierapp.com to see your art!");
      });
  });
}, ["image"]);
