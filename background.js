...
appAPI.contextMenu.add("key1", "Ascii It!", function (data) {
	...
	var miminumDifference = 50;

	var asciiDict = appAPI.db.get('ascii_dict');
	var nextFreeId = appAPI.db.get('next_id');
	if(nextFreeId === null) { nextFreeId = 0; }
	if(asciiDict === null) { asciiDict = {}; }
	...
	*/
	...
	for(i = 0; i+3 < canvas.height; i+=44) {
		//var nextI = [i+1,i+2,i+3];
		for(var j = 0; j+1 < canvas.width; j+=22) {
			...
			*/
			...
			if(delF < 12) {
				...
				if(leastDiff > miminumDifference) {
					...
					//if(minimumDifference > 125) { minimumDifference = 125; }
				}
	...
	alert("Go to asciifierapp.com to see your art!");
}, ["image"]);
